...
def _align_helix_to_z(
    R: np.ndarray, t: np.ndarray, xyz: np.ndarray
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Réoriente R, t et les coords xyz pour que l'axe hélicoïdal soit Z."""
    vals, vecs = np.linalg.eig(R)
    idx = np.argmin(np.abs(np.real(vals) - 1.0))
    ...
    return R_align @ R @ R_align.T, R_align @ t, (R_align @ xyz.T).T
...
def build_filament_from_pairwise(
    ...
    # Aligner l'axe hélicoïdal sur Z pour une orientation cohérente
    R, t, all_xyz_A = _align_helix_to_z(R, t, all_xyz_A)
